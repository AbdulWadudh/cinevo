...
export interface EpisodeNotification {
  id: string; // `${mediaId}:${season}:${episode}`
  mediaId: string;
  title: string;
...
/**
 * New & upcoming episodes for the TV shows in a profile's wishlist.
 * "new" = aired in the last 21 days; "upcoming" = airing in the next 21 days.
 * Shared by the in-app bell and the Web Push cron.
 */
export async function computeEpisodeNotifications(profileId: string): Promise<EpisodeNotification[]> {
  const shows = await db.wishlist.findMany({
    where: { profileId, mediaType: "tv" },
