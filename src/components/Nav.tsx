    ...
    <>
      ...
      >
        <div className="flex items-center gap-10">
          <Link href="/" className="font-display text-2xl font-extrabold tracking-wider text-accent uppercase hover:scale-105 transition-transform duration-200">
            Cinevo
          </Link>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-accent to-purple-600 shadow-[0_2px_10px_rgba(229,62,79,0.3)] cursor-pointer" />
        </div>
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 bg-bg/95 backdrop-blur-2xl z-50 overflow-y-auto animate-fade-in flex flex-col px-6 md:px-12 pt-20 pb-12">
                  ...
                  {filteredResults.map((item) => {
                    ...
                    const imageUrl = item.poster_path || item.backdrop_path 
                      ...
                      : (item as any).profile_path
                        ? `https://image.tmdb.org/t/p/w200${(item as any).profile_path}`
                        : "https://picsum.photos/seed/cinevodefault/200/300";

                    const linkUrl = `/watch/${item.media_type}/${item.id}`;
                    ...
                    return (
                      <div
                        ...
                        className="flex items-center gap-4 bg-surface hover:bg-surface-hover border border-border hover:border-white/[0.08] p-3 rounded-xl transition-all group cursor-pointer"
                      >
                        {/* Image Thumbnail */}
                        <div className="relative w-14 aspect-[2/3] bg-bg rounded-lg overflow-hidden flex-none">
                          <img 
                            src={imageUrl} 
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="text-sm font-bold truncate text-fg group-hover:text-accent transition-colors">
                            {item.title || item.name}
                          </h4>
                          <p className="text-xs text-muted flex items-center gap-1.5 mt-1 font-medium">
                            {isPerson ? (
                              <>
                                <User className="w-3.5 h-3.5 text-accent" />
                                <span>Actor / Crew (Click to view filmography)</span>
                              </>
                            ) : (
                              <>
                                <Film className="w-3.5 h-3.5 text-accent" />
                                <span>{item.media_type === "tv" ? "TV Show" : "Movie"}</span>
                                {item.release_date && <span>&bull; {item.release_date.split("-")[0]}</span>}
