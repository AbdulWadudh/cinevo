...
export const StaggeredMenu = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(icon, { scale: 1, opacity: 1, transformOrigin: "50% 50%" });
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (footerEl) {
      gsap.set(footerEl, { y: 25, opacity: 0 });
    }
    ...
    if (footerEl) {
      ...
      tl.to(footerEl, { y: 0, opacity: 1, duration: 0.5, ease: "power3.out" }, footerStart);
    }
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    // The icon glyph swaps (hamburger ↔ X) via state; quick upright pop only.
    spinTweenRef.current = gsap.fromTo(
      icon,
      { scale: 0.7, opacity: 0.4 },
      { scale: 1, opacity: 1, duration: 0.28, ease: "power3.out", overwrite: "auto" }
    );
  }, []);
    ...
    >
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <div className="sm-panel-inner">
          ...
          {panelFooter && <div className="sm-panel-footer">{panelFooter}</div>}
        </div>
      </aside>
    </div>
  );
