...
- **Docker image and Coolify deployment path.** A three-stage `Dockerfile` on Next.js `output: "standalone"` — install (with `prisma/` copied ahead of `npm ci`, since `postinstall` runs `prisma generate` and needs the schema), build, then an Alpine runtime that carries only the traced server, `.next/static`, `public/` and the generated Prisma client, running as a non-root user. `docker-compose.yml` builds the same image locally off `.env` so a deploy can be rehearsed before it's pushed.
  - **`NEXT_PUBLIC_*` values are build arguments, not runtime env.** `next build` inlines them into the browser bundle, so the Dockerfile declares an `ARG` for each and Coolify has to mark them *Build Variable*; supplying them only at run time leaves the client with an empty Supabase URL and no working sign-in. TMDB credentials join them, because `/gallery` and `/reveal` prerender against TMDB during the build. The runtime-only half — `SITE_URL`, the VAPID private key, `PUSH_CRON_SECRET` — can change without a rebuild, which is why `getSiteURL` prefers `SITE_URL` over its public twin.
  - The generated Prisma client is copied into the runtime stage explicitly. Prisma 7 reaches its WASM query compiler through a runtime `require()`, and while the standalone tracer does pick it up here, the copy costs a few MB and removes the possibility of a first-query crash on a future Prisma release.
  - **`/api/health`** liveness route for Coolify's health check and the image's own `HEALTHCHECK`. It touches neither Postgres nor Supabase on purpose: a probe that queried them would report a wobbling upstream as an unhealthy container and get a perfectly good server restarted. It's also excluded from the `proxy.ts` matcher, which otherwise spends a Supabase `getUser()` call on every request — thirty seconds apart, that's a needless round-trip and a needless way for the probe to fail.
