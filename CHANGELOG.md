...
- **Docker image and Coolify deployment path.** A two-stage `Dockerfile` on Next.js `output: "standalone"`: a builder that installs from the manifests first (with `prisma/` copied ahead of `npm ci`, since `postinstall` runs `prisma generate` and needs the schema) and then builds, and a runtime carrying only the traced server, `.next/static` and `public/`, running as the `node` user. `docker-compose.yml` builds the same image locally off `.env` so a deploy can be rehearsed before it's pushed.
  - **Debian slim, not Alpine.** Next's SWC binary, `lightningcss` and `@tailwindcss/oxide` each ship a separate musl build, and that's a class of install failure not worth carrying to save ~40MB — which also retires the `libc6-compat` shim each stage needed. Pinned to Node 26.
  - **`NEXT_PUBLIC_*` values are build arguments, not runtime env.** `next build` inlines them into the browser bundle, so the Dockerfile declares an `ARG` for each; supplying them only at run time leaves the client with an empty Supabase URL and no working sign-in. The TMDB pair joins them because `/gallery` and `/reveal` prerender against the API. Coolify injects its own `ARG`/`ENV` per variable, so those declarations exist to keep a plain `docker build` equivalent. `DATABASE_URL` is *not* among them — no prerendered route touches Postgres — and the rest of the runtime half (`SITE_URL`, the VAPID private key, `PUSH_CRON_SECRET`) can change without a rebuild, which is why `getSiteURL` prefers `SITE_URL` over its public twin.
  - **`/api/health`** liveness route for Coolify's health check. It touches neither Postgres nor Supabase on purpose: a probe that queried them would report a wobbling upstream as an unhealthy container and get a perfectly good server restarted. It's also excluded from the `proxy.ts` matcher, which otherwise spends a Supabase `getUser()` call on every request — thirty seconds apart, that's a needless round-trip and a needless way for the probe to fail. The image declares the `HEALTHCHECK` itself rather than relying on Coolify's panel settings, because the Docker Compose build pack reads the check from the Dockerfile (or a compose `healthcheck:` block) and ignores the panel — without it an unhealthy container keeps taking traffic. Compose inherits the image's check, so it isn't repeated there. The probe runs through Node's global `fetch`, so it doesn't depend on `curl` or `wget` surviving in the base image.
  - Migrations stay out of the container — no Prisma CLI in the runtime image, so `prisma migrate deploy` is run from a workstation or CI against `DIRECT_URL` rather than by every replica racing each other on boot.
  - `vercel.json`'s daily `crons` entry has no equivalent on Coolify; the README now documents the Scheduled Task that replaces it, calling `/api/push/run` with the `PUSH_CRON_SECRET` bearer token the route already accepts.
...
- **The Docker build couldn't install dependencies — two separate lockfile faults.** Both surfaced as `EUSAGE`, *"`npm ci` can only install packages when your package.json and package-lock.json are in sync"*, before the build began.
  - A Windows `npm install` had rewritten `package-lock.json`, dropping 2,514 lines and 78 packages: every `linux`/`darwin` platform build of `@next/swc`, `@tailwindcss/oxide`, `lightningcss`, `@unrs/resolver-binding` and `sharp`, leaving only `win32-x64-msvc`. `package.json` was untouched, so the lockfile was restored from the preceding revision rather than regenerated, keeping every resolved version identical.
  - That exposed a second, older gap: the wasm32 fallbacks of `@tailwindcss/oxide` and `sharp` need a hoisted `@emnapi/core` and `@emnapi/runtime`, which the lockfile has never carried. npm run from Windows will not add them — not with `--package-lock-only`, and not with `--os=linux --cpu=x64` either — so no lockfile committed from this machine can satisfy `npm ci` on Linux. The image therefore installs with `npm install --no-audit --no-fund`, which honours every version the lockfile does pin and resolves only the gaps; it's also what Vercel has run by default all along, which is why the gap never showed up there. Restoring `npm ci` means generating the lockfile on Linux.
