...
- **Profile rebuilt as a settings workspace:**
  ...
  - The rail carries identity (avatar, name, email, role chip, join month), the Wishlist / History stat tiles, and grouped section nav — **Account:** Profile, Appearance, Data & Sync; **Admin:** Overview, Providers, Reports (badged with the open count), Radio. Sign out sits at the foot of the rail on desktop and below the pane on phones, so it is reachable from every section rather than only from the bottom of the page.
