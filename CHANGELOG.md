...
- **`npm ci` failed on Linux — the lockfile had lost every non-Windows binary.** A Windows `npm install` rewrote `package-lock.json`, dropping 2,514 lines and 78 packages: all the `linux`/`darwin` platform builds of `@next/swc`, `@tailwindcss/oxide`, `lightningcss`, `@unrs/resolver-binding` and `sharp`, leaving only the `win32-x64-msvc` ones. `npm install` tolerates that — it just resolves what's missing — but `npm ci` refuses (`EUSAGE`, *"can only install packages when your package.json and package-lock.json are in sync"*), so every Docker build died before it started. `package.json` was untouched, so the lockfile was restored from the preceding revision rather than regenerated, keeping every resolved version identical. Verified with `npm ci --dry-run`.
